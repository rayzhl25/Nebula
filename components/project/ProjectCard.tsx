...
const ProjectCard: React.FC<ProjectCardProps> = ({ 
  ...
  return (
    <div className="group bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-nebula-500 dark:hover:border-nebula-500 transition-all hover:shadow-lg flex flex-col h-full relative overflow-hidden">
       {/* 顶部颜色条 */}
                ...
                <div className="dropdown relative group/menu">
                  ...
                  <div className="hidden group-hover/menu:block absolute right-0 top-full pt-1 w-32 z-20">
                     <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl py-1">
                       <button onClick={() => onEdit(project)} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 flex items-center gap-2">
                         <Edit size={14} /> {t.edit}
                       </button>
                       ...
                       <button onClick={() => onViewKey(project)} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 flex items-center gap-2">
                         <Key size={14} /> {t.viewKey}
                       </button>
                       <button onClick={() => onPublish(project)} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 flex items-center gap-2">
                         <LayoutTemplate size={14} /> {t.publishTemplate}
                       </button>
                       <button onClick={() => onExport(project)} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 flex items-center gap-2">
                         <Download size={14} /> {t.exportProject}
                       </button>
                       <button onClick={() => onDelete(project)} className="w-full text-left px-4 py-2 text-sm hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 flex items-center gap-2">
