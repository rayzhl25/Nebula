...
const Login: React.FC<LoginProps> = ({ onLogin, lang, setLang, theme, setTheme }) => {
  ...
  return (
    <div className="min-h-screen transition-colors duration-500 bg-gray-50 dark:bg-gradient-to-br dark:from-indigo-900 dark:via-purple-900 dark:to-black flex items-center justify-center px-4 relative">
      ...
      <div className="absolute top-4 right-4 flex items-center gap-3 z-10">
         ...
         <div className="flex bg-white dark:bg-white/10 rounded-full p-1 shadow-sm border border-gray-200 dark:border-white/10">
             ...
             <button onClick={() => setTheme('system')} className={`p-1.5 rounded-full transition-all duration-300 ${theme === 'system' ? 'bg-gray-100 dark:bg-white text-blue-500 shadow-sm' : 'text-gray-400 dark:text-gray-300 hover:text-gray-600 dark:hover:text-white'}`}><Monitor size={16} /></button>
      ...
      <div className="bg-white dark:bg-white/10 dark:backdrop-blur-lg border border-gray-200 dark:border-white/20 p-8 rounded-2xl w-full max-w-md shadow-xl dark:shadow-2xl transition-all duration-500">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-purple-600 rounded-xl flex items-center justify-center text-white font-bold text-3xl mx-auto mb-4 shadow-lg shadow-purple-500/30">
            N
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-lg bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition-all"
              placeholder="admin"
            />
          </div>
          <div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-4 rounded-lg transition-colors shadow-lg shadow-purple-600/30 dark:shadow-purple-900/50 flex items-center justify-center"
          >
